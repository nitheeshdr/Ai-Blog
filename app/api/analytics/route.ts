...
export async function GET() {
  ...
  // Run all queries with individual error handling so missing tables don't crash
  const [postsRes, viewsRes, subscribersRes, logsRes, topicsRes] = await Promise.allSettled([
  ...
  return NextResponse.json({
    ...
    data: {
      ...
      totalViews,
      avgSeoScore,
      totalSubscribers: subscribersRes.status === "fulfilled" ? (subscribersRes.value.count ?? 0) : 0,
      pendingTopics: topicsRes.status === "fulfilled" ? (topicsRes.value.count ?? 0) : 0,
      recentLogs: logs,
    },
  });
}
